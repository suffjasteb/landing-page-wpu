...
{{-- <article class="py-8 max-w-screen-md border-b border-gray-500">
     <a href="{{ url('/posts/' . $post['slug']) }}" class="hover:underline">
      ...
      <div class="text-gray-500">
        <a href="/belajar-laravel/public/authors/{{ $post->author->username }}" class="hover:underline text-base text-gray-500">{{ $post->author->name }}</a> 


        <a href="/belajar-laravel/public/categories/{{ $post->category->slug }}" class="hover:underline text-base text-blue-500">{{ $post->category->name }}</a> | {{ $post->created_at->format('j F Y')}}


      </div>
      <p class="my-4 font-light">{{ Str::limit( $post['body'], 150) }}</p>
      <a href="{{ url('/posts/' . $post['slug']) }}" class="font-medium text-blue-500">Read more &raquo;</a>
    </article> --}}
